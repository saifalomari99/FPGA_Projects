...
void led_chase1 (GpoCore *led_p, int n, XadcCore *adc_p){
	int i, j;
	int enable = 0; //for changing directions
	static int speed; // How fast the light moves
	static int prevSpeed; //Previous set speed (for preventing multiple Debug commands)
	uint16_t raw;
	double myADC;
	myADC = adc_p->read_adc_in(0);
	...
	for (i = 0; i < n; i++) {
		...
		prevSpeed = speed;
		speed = myADC * 750; ; //&63 is for preventing other switches as inputs
...
/* disp_adc_sseg displays ADC value ( 0 to 1) on lower three SSEG display*/
void disp_adc_sseg(SsegCore *sseg_p,XadcCore *adc_p){
	double value;
	int a,b,c; //holds values of ones, tenths, and hundreths place

	value = adc_p->read_adc_in(0); //reads adc value
	a = value / 1;
	b = int(value *10) % 10;
	c = int(value *100) % 10;
	sseg_p->set_dp(1 << 2); //sets decimal point to the third sseg
	sseg_p->write_1ptn(sseg_p->h2s(a), 2);//convert hex to sseg values and display in sseg 2
	sseg_p->write_1ptn(sseg_p->h2s(b), 1);
	sseg_p->write_1ptn(sseg_p->h2s(c), 0);
}

void rgb_adc_control(PwmCore *pwm_p, XadcCore *adc_p) {
   double adc, bright, b, g , r; // goes in order of bgr
   adc = adc_p->read_adc_in(0);//read adc
   bright = 0.2; //set brightness
   pwm_p->set_freq(50); //sets freq of pwm

   if (adc > 0 && adc < 1.0/6) {
       b = 0.0;
       g = pwm_p->map(adc, 0, 1.0/6, 0, 1);//need this for setting the correct range for green
       r = 1.0;
   }
   if (adc >= 1.0/6 && adc < 2.0/6) {
       b = 0.0;
       g = 1.0;
       r = pwm_p->map(adc, 1.0/6, 2.0/6, 1, 0);
   }
   if (adc >= 2.0/6 && adc < 3.0/6) {
       b = pwm_p->map(adc, 2.0/6, 3.0/6, 0, 1);
       g = 1.0;
       r = 0.0;
   }
   if (adc >= 3.0/6 && adc < 4.0/6) {
       b = 1.0;
       g = pwm_p->map(adc, 3.0/6, 4.0/6, 1, 0);
       r = 0.0;
   }
   if (adc >= 4.0/6 && adc < 5.0/6) {
       b = 1.0;
       g = 0.0;
       r = pwm_p->map(adc, 4.0/6, 5.0/6, 0, 1);
   }
   if (adc >= 5.0/6 && adc < 1) {
       b = pwm_p->map(adc, 5.0/6, 1, 1, 0);
       g = 0.0;
       r = 1.0;
   }
   pwm_p->set_duty(b * bright, 0);
   pwm_p->set_duty(g * bright, 1);
   pwm_p->set_duty(r * bright, 2);
   pwm_p->set_duty(b * bright, 3);
   pwm_p->set_duty(g * bright, 4);
   pwm_p->set_duty(r * bright, 5);
   //pwm_p->set_duty(0.0, 1 + 3);
}
...
//      show_test_id(5, &led);
      //adc_check(&adc, &led);
	  //led_chase1(&led,16,&adc);
	  disp_adc_sseg(&sseg,&adc);
	  rgb_adc_control(&pwm, &adc);
//      show_test_id(6, &led);
