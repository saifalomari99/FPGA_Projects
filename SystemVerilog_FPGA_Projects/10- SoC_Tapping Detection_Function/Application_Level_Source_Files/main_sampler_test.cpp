...
//void gsensor_check(SpiCore *spi_p, GpoCore *led_p) { //prof given gsensor_check
...
void gsensor_check(SpiCore *spi_p, GpoCore *led_p) {
   ...
   const uint8_t DATA_REG = 0x08;
   const float raw_max = 127.0 / 2.0;  //128 max 8-bit reading for +/-2g
   double mag; //holds the magnitude of tapping
   int ledRange; //holds number of leds that need to be turned on depending on mag
   double stable; //the stable acc magnitude of the board

   int8_t xraw, yraw, zraw; //these values will be used to determine the mag of the tapping
   float x, y, z; 

   int8_t xxraw, yyraw, zzraw; //these values will hold the stable mag of the board to prevent random LEDs from glowing
   float xx,yy,zz;			   
   ...
   //This section reads the acceleration for the stable variable
   spi_p->assert_ss(0);    // activate
   spi_p->transfer(RD_CMD);  // for read operation
   spi_p->transfer(DATA_REG);  
   xxraw = spi_p->transfer(0x00);
   yyraw = spi_p->transfer(0x00);
   zzraw = spi_p->transfer(0x00);
   spi_p->deassert_ss(0);
   xx = (float) xxraw / raw_max;
   yy = (float) yyraw / raw_max;
   zz = (float) zzraw / raw_max;
   stable = fabs(xx) + fabs(yy) + fabs(zz); //306-307 get the "stable" acceleration of
   sleep_ms(1); 			    //the board so that the angle doesn't affect
   	   	   	   	   	    //the value of mag and give inaccurate LED readings

   //This section will save the acceleration of the tapping
   spi_p->assert_ss(0);    // activate
   spi_p->transfer(RD_CMD);  // for read operation
   spi_p->transfer(DATA_REG);  //
   xraw = spi_p->transfer(0x00);
   ...
   y = (float) yraw / raw_max;
   z = (float) zraw / raw_max;
   mag = fabs(x)+fabs(y)+fabs(z); // saves the mag of the tapping
	
   if(mag > stable){ //makes sure stable value is correct
	   ledRange = map(mag,stable,stable*3,0,15);//maps leds
	   if (ledRange>15)
		   ledRange =15;
	   if (ledRange<0)
		   ledRange=0;
   	   ...
   	   led_p->write(0);//after a tap, turn off all LEDS
   	}
