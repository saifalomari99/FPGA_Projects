...
public:
   ...
   // ------------ method to read a 32-bit word
   uint32_t read();

   // ---------- method to read a bit at a specific position
...
public:
   ...
   enum {
	  ...
	  DATA_REG1 = 1,                // Data registers of the intervals durations
	  DATA_REG2 = 2,
	  DATA_REG3 = 3
   };

   //-------- constructor
   BlinkingCore(uint32_t core_base_addr);
   ~BlinkingCore();


   // --------- blink method
   void write_reg(uint32_t rate, int bit_pos);
