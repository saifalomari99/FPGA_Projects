...
//------------------------------------------------- original test functions: 14 in total
...
void chase_LED_adc (GpoCore *led_p, GpiCore *sw_p, int n , XadcCore *adc_p);
void chase_LED_keyboard (GpoCore *led_p, SsegCore *sseg_p, Ps2Core *ps2_p);
int get_speed_keyboard (SsegCore *sseg_p, Ps2Core *ps2_p);
...
int main()
{
   ...
   while (1)
   {
	   if (sw13 == 1)                //---------------------------------- sw13
	   {
		   chase_LED_keyboard (&led,  &sseg, &ps2);
	   }
	   else if (sw14 == 1)           //---------------------------------- sw14
	   {
		   ps2_check(&ps2);
...
void chase_LED_keyboard (GpoCore *led_p, SsegCore *sseg_p, Ps2Core *ps2_p)
{
	...
	char ch;
	static int paused = 0;
	sseg_p->write_1ptn(0x12, 5);                                              // display letter s
	sseg_p->write_1ptn(0x0c, 4);                                              // display letter p
	sseg_p->write_1ptn(0x37, 3);                                              // display sign =
	...
	if (direction == 0)//---------------------------------------------------- right direction
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			if (ps2_p->get_kb_ch(&ch))                                       // if there is a keyboard input, save it into ch
			{
	            uart.disp(ch);
	            uart.disp(" ");
				if (ch == 'p')                                               // if the key is p == pause/resume
				{
					if (paused == 0)
						paused = 1;
					else if (paused == 1)
				...
				if (ch == -16)                                               // If the user presses F1 Key
				{
					uart.disp("F1 pressed ");
					speed = get_speed_keyboard(sseg_p, ps2_p);
				}
			}
			...
			if (paused)                                                     // pause
			{
				sseg_p->write_1ptn(0x0c, 7);                                // display p (pause) on the 8th 7-segment display.
				sseg_p->set_dp(1 << 7);
				led_p->write(1, 0);                                         // set led0 to be on
				direction = 0;
				i = 0;
				sleep_ms(10);
			}
			...
			sseg_p->write_1ptn(sseg_p->h2s((speed / 100) % 10), 2);                   // Display hundreds
			sseg_p->write_1ptn(sseg_p->h2s((speed / 10)  % 10), 1);                   // Display tens
			sseg_p->write_1ptn(sseg_p->h2s( speed        % 10), 0);                   // Display units
			//--------------------------
		}//---- end loop
	...
	else//-------------------------------------------------------------------- Left direction
	{
		...
		for (i = n-2; i > 0; i--)                                            //from 14 to 1 for smooth transition
		{
			...
			if (paused)
			{
				sseg_p->write_1ptn(0x0c, 7);                                        // display the letter p
				sseg_p->set_dp(1 << 7);
				led_p->write(1, 0);
				direction = 0;
				i = 0;
...
int get_speed_keyboard (SsegCore *sseg_p, Ps2Core *ps2_p)
{
	...
	static int speed_hundreds = 0;
	static int speed_tens = 0;
	static int speed_ones = 0;
	...
	while (1)
	{
		...
		if (ps2_p->get_kb_ch(&ch))
		{
			...
			if (digit_count < 3)
			{
				if ((ch=='0'||ch=='1'||ch=='2'||ch=='3'||ch=='4'||ch=='5'||ch=='6'||ch=='7'||ch=='8'||ch=='9') && digit_count==0)
				{
					speed_hundreds = (int)ch - 48;
					uart.disp("speed_hundreds: ");
					uart.disp(speed_hundreds);
					uart.disp(" ");
					digit_count++;
				}
				else if ((ch=='0'||ch=='1'||ch=='2'||ch=='3'||ch=='4'||ch=='5'||ch=='6'||ch=='7'||ch=='8'||ch=='9') && digit_count==1)
				{
					speed_tens = (int)ch - 48;
					uart.disp("speed_tens: ");
					uart.disp(speed_tens);
					uart.disp(" ");
					digit_count++;
				}
				else if ((ch=='0'||ch=='1'||ch=='2'||ch=='3'||ch=='4'||ch=='5'||ch=='6'||ch=='7'||ch=='8'||ch=='9') && digit_count==2)
				{
					speed_ones = (int)ch - 48;
					uart.disp("speed_ones: ");
					uart.disp(speed_ones);
					uart.disp(" ");
					digit_count++;
				}
		...
		speed = (speed_hundreds*100) + (speed_tens*10) + (speed_ones);
...
// --------------------------------------------------- function to test the ps2 (keyboard or mouse)
void ps2_check(Ps2Core *ps2_p) {
   int id;
   ...
   // test what type of device is connected:
   uart.disp("\n\rPS2 device (1-keyboard / 2-mouse): ");
   id = ps2_p->init();
   uart.disp(id);
   uart.disp("\n\r");
	...
	//-------------------------------------selected color--------------------------------------------------------------
	 ...
	 //---------------------------------- Changing Color -------------------------------------------------------------
			...
			if (changing_color == 4)       // ------------------------------------------- green
			{
				...
				if (counter_green < 0)
				{
					counter_green = 0;                      // Or handle wrap-around as required
				}
			...
			if (changing_color == 5)      // --------------------------------------------- red
			{
				...
				if (counter_red < 0)
				{
					counter_red = 0;                      // Or handle wrap-around as required
				}
