...
int Ps2Core::rx_fifo_empty() {
   ...
   rd_word = io_read(base_addr, RD_DATA_REG);         // read register 0
   empty = (int) (rd_word & RX_EMPT_FIELD) >> 8;      // do the unpacking of the data
   return (empty);                                    // return 1: if empty; 0: otherwise
}
...
int Ps2Core::tx_idle() {
   ...
   return (idle);                                     // return 1: if idle; 0: otherwise
}
...
{
   io_write(base_addr, PS2_WR_DATA_REG, (uint32_t) cmd);         // send the 8-bit data
}
...
// [6] --------------------------------------------------------------------- method to initialize the Ps2 Core 
/* procedure:
...
// [8] ---------------------------------------------------------------------
// method to get keyboard charctar
...
#define F12     0xfb
   ...
   // 1D array 
   // for example if we want to translate the letter s 
   // from scan code to ASCII:
   // in scan code s = 0x1b = 0001_1011 --> which is 22
   static const uint8_t SCAN2ASCII_LO_TABLE[128] = {
         0, F9, 0, F5, F3, F1,   F2, F12,        //00
         0, F10, F8, F6, F4, TAB, '`', 0,        //08
         0, 0, SFT_L, 0, CTR_L, 'q', '1', 0,     //10
         0, 0, 'z', 's', 'a', 'w', '2', 0,       //18, 19, 1a, 1b, 1c, 1d, 1e, 1f
         0, 'c', 'x', 'd', 'e', '4', '3', 0,     //20, 21, 22, 23, 24, 25, 26, 27
         0, ' ', 'v', 'f', 't', 'r', '5', 0,     //28, 29, 2a, 2b, 2c, 2d, 2e, 2f
         0, 'n', 'b', 'h', 'g', 'y', '6', 0,     //30
         0, 0, 'm', 'j', 'u', '7', '8', 0,       //38
         0, ',', 'k', 'i', 'o', '0', '9', 0,     //40
         0, '.', '/', 'l', ';', 'p', '-', 0,     //48
